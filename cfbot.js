...
module.exports = {
    ...
    run: async (client, message, args) => {
        ...
        //--problem
            ...
            let _Prommise = new Promise(async (resolve, reject) => {
                Codeforces.problemset.problems({ tags: tag }, function (err, problemSet) {

                    var problems = problemSet.problems;
                    if (problems) {
                        console.log(problems.length)
                        for (i in problems) {
                            if (problems[i].rating < 1000) {
                                veryEasy.push(problems[i]);
                            }
                            else if (problems[i].rating >= 1000 && problems[i].rating < 1200) {
                                easy.push(problems[i]);
                            }
                            else if (problems[i].rating >= 1200 && problems[i].rating < 1500) {
                                medium.push(problems[i]);
                            }
                            else if (problems[i].rating >= 1600 && problems[i].rating < 2000) {
                                hard.push(problems[i]);
                            }
                            else if (problems[i].rating >= 2000) {
                                veryHard.push(problems[i]);
                            }
                        }
                    }
                    else {
                        message.channel.send("Please Enter valid Tags.");
                        return;
                    }
                    resolve(" ");
                });
            ...
            let values = await Promise.all(promises);
            var probSet = [];
            probSet.push(veryEasy[getRandomArbitrary(veryEasy.length)]);
            probSet.push(easy[getRandomArbitrary(easy.length)]);
            probSet.push(medium[getRandomArbitrary(medium.length)]);
            probSet.push(hard[getRandomArbitrary(hard.length)]);
            probSet.push(veryHard[getRandomArbitrary(veryHard.length)]);
            console.log(probSet);
            var miss = " \n";
            var count = 1;
            for (i in probSet) {
                miss = miss + `**${count}. ${probSet[i].name} (Rating: ${probSet[i].rating})** ` + ` [Click here to Solve](https://codeforces.com/contest/${probSet[i].contestId}/problem/${probSet[i].index})` + "\n\n";
                count++;
            }
            var avatarURL = "https://news.itmo.ru/images/news/big/917925.jpg";
            const misscontest = new MessageEmbed()
                .setColor('#FFFF00')
                .setTitle("Here is your Problem Set")
                .setThumbnail(avatarURL)
                .setDescription(miss)
                .setTimestamp()
                //.addField("hi", "title")
